...
function drawSums(q){
  if(q.kind === 'sums'){
    const ask = q.shape === 0
      ? 'Колко различни числа можем да получим при сбор на ' + BGNUM[q.k] + ' едноцифрени числа?'
      : q.shape === 1
      ? 'Колко различни числа можем да получим при сбор на две числа, всяко от които не е по-голямо от <span class="num">' + q.n + '</span>?'
      : 'Колко различни числа можем да получим при разлика на две едноцифрени числа?';
...
function eqSums(q){
  if(q.kind === 'sums') return 'от 0 до ' + q.top + ' → ' + q.ans;
}
function whySums(q, full){
  if(q.kind === 'sums'){
    if(!full) return 'Кой е най-малкият и кой най-големият резултат?';
    return 'най-малкото е <b>0</b>, най-голямото е <b>' + q.top + '</b> &nbsp;→&nbsp; от 0 до ' + q.top +
      ' &nbsp;(с нулата) &nbsp;→&nbsp; ' + q.ans;
  }
}
